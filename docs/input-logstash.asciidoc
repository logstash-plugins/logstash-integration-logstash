...
Doing so requires connecting `logstash-output` plugin clients to provide matching <<plugins-{type}s-{plugin}-username>> and <<plugins-{type}s-{plugin}-password>>.
...
When this input plugin is configured with a `username`, it also requires a <<plugins-{type}s-{plugin}-password>>, and any upstream `logstash-output` plugin must also be configured with a matching `username`/`password` pair.
