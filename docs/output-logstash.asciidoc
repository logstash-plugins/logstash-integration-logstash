...
A downstream input {ls} hosts or IP addresses to connect.

Host can be any of IPv4, IPv6 (in enclosed bracket) or host name, examples:

* `"127.0.0.1"` or `["127.0.0.1"]` if single host
* `"127.0.0.1:9801"` or `["127.0.0.1:9801"]` if single host with port
* `["ds.example.com", "ds.example:9802"]`
* `"[::1]"`
* `"[::1]:9803"`
* `["[::1]:9801", "[::1]:9811"]`
