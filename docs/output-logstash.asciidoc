...
Configure this output plugin needs to connect to a {logstash-ref}/plugins-inputs-logstash.html[Logstash input plugin] by specifying its <<plugins-{type}s-{plugin}-hosts>>.
...
If the downstream `logstash-input` plugin is configured to require <<plugins-{type}s-{plugin}-username>> and <<plugins-{type}s-{plugin}-password>>,
