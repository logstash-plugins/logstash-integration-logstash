...
It does so by ensuring that the server presents a currently-valid certificate with identity claims matching <<plugins-{type}s-{plugin}-hosts>>, signed by a trusted signing authority, along with proof-of-possession of the associated private key material.
...
* Constraints:
** When IPv6 is applied, IP address must be in an enclosed bracket.
** When any URL omits a port component, the default `9800` is used.

A downstream input {ls} host or IP address to connect.

NOTE: Currently, plugin supports a single host. Multi-host support is coming soon.
